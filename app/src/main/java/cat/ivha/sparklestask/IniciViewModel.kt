...
class IniciViewModel : ViewModel() {
    ...
    private fun validateEmail(email: String) : Boolean{
        return when {
            ...
            !Patterns.EMAIL_ADDRESS.matcher(email).matches() -> {
                ...
                _emailError.value = null // Limpia el error
                true
            }
        }
    }
    ...
    private fun validatePassw(passw: String): Boolean{
        return when{
            passw.isEmpty() -> {
                _passwError.value = "Vigila! T'en oblidas la contrasenya"
                false
            }
            else -> {
    ...
    fun procesLogin(email: String, passw: String){
